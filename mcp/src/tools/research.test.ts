...
describe("registerResearchTools", () => {
  ...
  it("registers without throwing", () => {
    ...
    expect(() => registerResearchTools(server, client)).not.toThrow();
  });
  ...
  it("can be called multiple times on different servers", () => {
    ...
    expect(() => registerResearchTools(server1, client)).not.toThrow();
    expect(() => registerResearchTools(server2, client)).not.toThrow();
  });
});
