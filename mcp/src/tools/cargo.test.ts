...
import { registerCargoTools } from "./cargo.js";

describe("registerCargoTools", () => {
  ...
  it("registers without throwing", () => {
    ...
    expect(() => registerCargoTools(server, client)).not.toThrow();
  });
  ...
  it("can be called multiple times on different servers", () => {
    ...
    expect(() => registerCargoTools(server1, client)).not.toThrow();
    expect(() => registerCargoTools(server2, client)).not.toThrow();
  });
});
